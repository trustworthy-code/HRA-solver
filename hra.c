...
void hra(int argc, char* argv[])
{
    ...
    {
        scanf("%d", &task_count, &task_dependency_count);
    }

    int* task_dependencies =(int*)malloc(3 * task_count * sizeof(int));
    ...
    for (int i = 0; i < task_count; i++)
    {
        ...
        {
            fscanf(fin, " %d %d", &second_cost, &second_cost);
        }
    ...
    int result = hra_solver(task_count, task_dependency_count, task_costs, task_dependencies, schedule);
