...
int hra_solver(int task_count, int task_dependency_count, int* task_costs, int* task_dependencies,
               int* schedule)
{
    ...
    for (int i = 0; i < task_dependency_count; i++)
    {
        ...
        combination_list[combination_list_index] =
            task_costs[2 * task_dependencies[task_dependency_index]] +
    ...
    for (int j = 1; (j < task_dependency_count) && (is_sorted == 0); j++)
    {
        is_sorted = 1;
        int index_j = 5 * j;

        for (int i = 0; i < task_dependency_count-j; i++)
        {
            int index_i = 5 * i;

            if (combination_list[index_i] > combination_list[index_i + 6])
            {
                ...
                combination_list[index_i] = combination_list[index_i + 6];
                combination_list[index_i + 1] = combination_list[index_i + 7];
                combination_list[index_i + 2] = combination_list[index_i + 8];
                combination_list[index_i + 3] = combination_list[index_i + 9];
                combination_list[index_i + 4] = combination_list[index_i + 10];

                combination_list[index_i + 6] = tmp_1;
                combination_list[index_i + 7] = tmp_2;
                combination_list[index_i + 8] = tmp_3;
                combination_list[index_i + 9] = tmp_4;
                combination_list[index_i + 10] = tmp_5;
    ...
    for (int i = task_dependency_count - 1; (i >= 0) && (satisfiable == 1); i--)
    {
        int* formula_tmp = (int*)malloc(2 * (formula_tmp_disjunction_count + 1) * sizeof(int));
        ...
        {
            formula_tmp_first_literal = first_task + 1;
            formula_tmp_second_literal = second_task + 1;
        }
        ...
        if ((first_task + 1) > formula_tmp_variable_count)
        {
            formula_tmp_variable_count = first_task + 1;
        }
        if ((second_task + 1) > formula_tmp_variable_count)
        {
            formula_tmp_variable_count = first_task + 1;
        }
        ...
        if (satisfiable == 1)
        {
            ...
            int* formula = (int*)malloc(2 * formula_tmp_disjunction_count * sizeof(int));
            for (int j = 0; j < formula_tmp_disjunction_count; j++)
            {
                int index_j = 2 * j;
    ...
    int result = two_satisfiability_solver(formula_variable_count,
